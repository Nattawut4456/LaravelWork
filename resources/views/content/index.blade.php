...
@section('content')
    ...
    <div class="mb-2">
        ...
        <a href="{{url('vote/')}}" role="button" class="btn btn-sm btn-primary">Show all vote</a>
    </div>
            ...
            @foreach ($contents as $content)
                <tr>
                    ...
                    <td>
                        ...
                        @endif
                        <a role="button" class="btn btn-primary vote-item" data-id="{{$content->id}}">Vote</a>
                    </td>
    ...
    <script>
        document.querySelector('#tbContent').addEventListener('click', (e) => {
            if (e.target.matches('.delete-item')) {
             ...
             } else if (e.target.matches('.vote-item')) {
                Swal.fire({
                    title: 'Vote?',
                    text: "asasdsadsad",
                    icon: 'question',
                    ...
                    denyButtonText: 'Dislike'

                }).then((result) => {
                    if (result.isConfirmed) {
                        axios.post($url +'/vote/' + e.target.dataset.id +'/like').then((response) => {
                            Swal.fire(
                                'Success!',
                                'You vote Like!',
                                'success'
                        ...
                        axios.post($url +'/vote/' + e.target.dataset.id +'/dislike').then((response) => {
                            Swal.fire(
                                'Success!',
                                'You vote Dislike!',
                                'success'
